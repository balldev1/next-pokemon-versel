    ...
    useEffect(() => {
        const fetchPokemonList = async () => {
            try {
                ...
                setFilteredPokemon(detailedData); // เริ่มต้นให้ filteredPokemon = ข้อมูลทั้งหมด
            } catch (error) {
        ...
        <div className="container mx-auto">
            <div className="text-2xl flex gap-5 font-bold mb-4">
                ...
                <div className="flex items-center gap-2">
                    ...
                    />
                    <button onClick={handleSearch} className="btn btn-accent btn-active
                    hover:opacity-90
                    ">
                        Confirm
                    </button>
                    <button onClick={handleReset} className="btn btn-accent btn-active
                    hover:opacity-90">
                        Default
                    </button>
                </div>
            ...
            {filteredPokemon.length === 0 ? (
                <div className="text-center  h-96 flex items-center justify-center text-red-500 text-lg font-bold">❌ ไม่พบข้อมูลโปเกมอน</div>
            ) : (
                <div className="grid grid-cols-4 gap-4 ">
                    {filteredPokemon.map((pokemon, index) => (
                        <Link key={index} href={`/pokemon/${pokemon.name}`} passHref className="hover:bg-accent/10 ">
                            <div
                                key={index}
